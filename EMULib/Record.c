...
/** replay.                                                 **/
/**                                                         **/
/** Copyright (C) Marat Fayzullin 2013                      **/
...
static unsigned int (*SaveState)(unsigned char *,unsigned int);
static unsigned int (*LoadState)(unsigned char *,unsigned int);
...
/** it back later.                                          **/
/*************************************************************/
int RPLRecord(unsigned int Cmd)
{
  int J;
  ...
  if(++RPLWCount<RPL_STEP)
  {
    /* If same joystick state, just increment counter */
    if(RPLData[WPtr1].JoyState[WPtr2]==Cmd)
    {
...
unsigned int RPLPlay(int Cmd)
{
  ...
  if(!RPLRCount)
  {
    ...
    if((RPtr2<0) || (++RPtr2>=RPL_RECSIZE) || !RPLData[RPtr1].Count[RPtr2])
    {
      ...
      /* Load next emulation state, if needed */
      if((RPtr2<0) && RPLData[RPtr1].State && RPLData[RPtr1].StateSize)
