...
int ShowVideo(void)
{
  ...
  if(!(Effects&(EFF_SOFTEN|EFF_SCALE|EFF_TVLINES|EFF_LCDLINES)))
  {
#ifdef MITSHM
  ...
  SX     = 0;
  SY     = 0;
  EFF    = Effects&(EFF_TVLINES|EFF_LCDLINES);
  ...
  if(Effects&EFF_SOFTEN)
  {
    ...
    /* Apply TV scanlines */
    if(EFF==EFF_TVLINES)       TelevizeImage(&OutImg,0,0,OutImg.W,OutImg.H);
    else if(EFF==EFF_LCDLINES) LcdizeImage(&OutImg,0,0,OutImg.W,OutImg.H);
    else if(EFF)               RasterizeImage(&OutImg,0,0,OutImg.W,OutImg.H);
  }
  else if(EFF)
  {
    ...
    else
    {
      ...
      /* Apply TV scanlines */
      if(EFF==EFF_TVLINES)       TelevizeImage(&OutImg,(OutImg.W-VideoW)>>1,(OutImg.H-VideoH)>>1,VideoW,VideoH);
      else if(EFF==EFF_LCDLINES) LcdizeImage(&OutImg,(OutImg.W-VideoW)>>1,(OutImg.H-VideoH)>>1,VideoW,VideoH);
      else if(EFF)               RasterizeImage(&OutImg,(OutImg.W-VideoW)>>1,(OutImg.H-VideoH)>>1,VideoW,VideoH);
    }
  }
...
int ProcessEvents(int Wait)
{
  ...
  /* @@@ TODO: Return 0 when application quits */
  return(1);
}
